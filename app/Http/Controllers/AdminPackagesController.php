<?php namespace App\Http\Controllers;
	...
	class AdminPackagesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Credit Point","name"=>"credit_point"];
			$this->col[] = ["label"=>"Price","name"=>"price"];
			$this->col[] = ["label"=>"Expires In Months","name"=>"expires_in_months"];
			$this->col[] = [
					"label"=>"Status",
					"name"=>"status",
			...
			$this->form[] = ['label'=>'Name ['.$locale.']','name'=>'title['.$labbr.']','type'=>'text','callback_php'=>'json_decode($row->title)->'.$labbr,'validation'=>'required|string|min:3|max:70|unique:packages','width'=>'col-sm-10','placeholder'=>'You can only enter the letter only'];
		    }
		    foreach (config('app.locales') as $labbr => $locale) {			
			$this->form[] = ['label'=>'Description ['.$locale.']','name'=>'description['.$labbr.']','type'=>'wysiwyg','callback_php'=>'json_decode($row->description)->'.$labbr,'validation'=>'required|min:1|max:5000','width'=>'col-sm-10'];
		    }
			...
			$this->form[] = ['label'=>'Credit Point','name'=>'credit_point','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Price','name'=>'price','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Expires In Months','name'=>'expires_in_months','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = [
				'label'=>'Status',
				'name'=>'status',
				'type'=>'select',
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
	        $request = Request::all();	
	        $slug = SlugService::createSlug(SeoUrl::class, 'slug',$request['title']['en']);
	        ...
	        CRUDBooster::redirect($request['return_url'],"Package Added !", "success");
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
	        ...
	        if(is_null($row->seo_url_id)){
		        ...
		        CRUDBooster::redirect($request['return_url'],"Package Updated !", "success");
	        }else{
	        	$package = Package::findOrFail($id);
		        $request['image'] = $postdata['image'];
		        $package->update($request);
